...
public class Open implements ICommand {
  ...
  public List<OutputMessage> execute(IGameContext gameContext, Map<Sequence, Word> words) {
    ...
    Availability containerAvailability = commandUtils
        .checkItemAvailability(openingItem, commandUtils.roomItems);
    Availability doorAvailability = commandUtils
        .checkDoorAvailability(openingItem, commandUtils.roomDoors);

    if (words.size() > 2) {
      outputMessages = executeOpenForMoreThanTwoWords(gameContext, openingItem, keyItem,
          containerAvailability, doorAvailability);
    } else {
      outputMessages = executeOpenForTwoOrLessWords(gameContext, openingItem,
  ...
  private List<OutputMessage> executeOpenForTwoOrLessWords(IGameContext gameContext,
      Word openingItem, Availability containerAvailability,
  ...
  private List<OutputMessage> executeOpenForMoreThanTwoWords(IGameContext gameContext,
      Word openingItem, Word keyItem,
      ...
      switch (doorAvailability) {
        case UNIQUE:
          if (keyItem != null) {
            ...
            outputMessages
                .add(new OutputMessage(1155, PrintStyle.ONLY_TITLE)); //item doesn't exist in inv.
            outputMessages.add(new OutputMessage(0, PrintStyle.BREAK));
          }
          break;
        case DUPLICATE:
  ...
  private List<OutputMessage> openItem(IGameContext gameContext, Word itemName) {
    ...
    outputMessages.add(
        new OutputMessage(tempItem.getId(), PrintStyle.ONLY_TITLE_IN_SAME_LINE));   // item to print
